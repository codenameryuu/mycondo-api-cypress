...
describe("Spec: get subcategory by id with invalid data", () => {
  beforeEach(() => {
    cy.login();
  });
  ...
  it("Should be failed, case: bearer access token is required", () => {
    cy.request({
      method: "GET",
      url: apiUrl + "/dashboard/subcategory/" + categoryId,
    }).then((response) => {
  ...
  it("Should be failed, case: category id is not found", () => {
    cy.request({
      method: "GET",
      url: apiUrl + "/dashboard/subcategory/999999999999999999999999",
      headers: {
        Authorization: `Bearer ${window.localStorage.getItem("access_token")}`,
      },
    }).then((response) => {
...
describe("Spec: get subcategory by id with valid data", () => {
  beforeEach(() => {
    cy.login();
  });
